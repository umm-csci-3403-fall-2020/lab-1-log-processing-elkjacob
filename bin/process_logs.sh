...
for i in "$@"
do
base_file_name=$(awk 'match($0, /(-w\)_secure.tgz/, base) {print base[1]}')
tar -xzf ../log_files/"$i" -C "$tmp_dir"/"$base_file_name"
#Call 'process_client_logs.sh'
./bin/process_client_logs.sh ./"$tmp_dir"/"$base_file_name"
done

create_username_dist.sh "$tmp_dir"
create_hours_dist.sh "$tmp_dir"
create_country_dist.sh "$tmp_dir"
assemble_report.sh "$tmp_dir"
